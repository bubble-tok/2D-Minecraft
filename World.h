...
public:
    ...
    void spawnDefaultEntities() {
        animals.push_back(std::make_shared<Animal>(300.f, (GROUND_ROW-1)*TILE_SIZE));
        animals.push_back(std::make_shared<Animal>(550.f, (GROUND_ROW-1)*TILE_SIZE));
        zombies.push_back(std::make_shared<Zombie>(900.f, (GROUND_ROW-1)*TILE_SIZE));
        zombies.push_back(std::make_shared<Zombie>(1200.f, (GROUND_ROW-1)*TILE_SIZE));
    }
    ...
    bool isSolid(float px, float py) const {
        return !getBlock((int)(py/TILE_SIZE), (int)(px/TILE_SIZE)).empty();
    }
    ...
    void update(float dt, Entity& player) {
        for (auto& a : animals) a->update(dt);
        for (auto& z : zombies) z->chaseAndAttack(player, dt);
        removeDeadEntities();
    }
